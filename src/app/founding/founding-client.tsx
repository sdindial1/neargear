...
import { AceCharacter, type AceState } from "@/components/ace/ace-character";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { FoundingBadge } from "@/components/founding-badge";
import { SparkleRain } from "@/components/founding/sparkle-rain";
import { ConfettiBurst } from "@/components/founding/confetti-burst";
...
export function FoundingClient({
  ...
  // Ace greets visitors excited, then settles to idle after 3s.
  const [aceState, setAceState] = useState<AceState>("excited");
  ...
  const remainingColorClass = isCritical
    ? "text-red-400"
    : isLow
      ? "text-orange"
      : "text-white";
  const progressColorClass = isCritical ? "bg-red-500" : "bg-orange";
  const counterPulseClass = isCritical
    ? "founding-counter-pulse-fast"
    : isLow
      ? "founding-counter-pulse-slow"
      : "";

  useEffect(() => {
    const timer = setTimeout(() => setAceState("idle"), ACE_EXCITED_MS);
  ...
  const handleClaim = async () => {
    ...
    if (res.ok) {
      ...
      // Celebrate — re-excite Ace and fire the burst.
      setAceState("excited");
      setShowBurst(true);
      setTimeout(() => setAceState("idle"), ACE_EXCITED_MS);
  ...
  return (
    <div className="relative isolate overflow-hidden">
      <div className="founding-gradient" aria-hidden />
      <SparkleRain />

      <main className="relative z-[2] px-4 pb-8">
        <section className="text-center pt-6 pb-8">
          <div className="relative inline-block">
            <div className="founding-glow" aria-hidden />
            <div className="relative">
              <AceCharacter state={aceState} size="lg" />
            </div>
          </div>
          <div className="mt-4 flex justify-center">
            <FoundingBadge shimmer />
          </div>
        ...
        <section className="bg-white/5 border border-white/10 rounded-2xl p-5 max-w-md mx-auto mb-8 backdrop-blur-sm">
          ...
          >
            <span className={`text-5xl ${counterPulseClass}`}>{remaining}</span>
        ...
        <section className="max-w-md mx-auto">
          {isFoundingMember ? (
            <div className="bg-green-500/10 border border-green-400/30 rounded-2xl p-6 text-center backdrop-blur-sm">
